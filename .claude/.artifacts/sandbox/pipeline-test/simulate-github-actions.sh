...
if python3 -m pytest tests/ -v --tb=short 2>/dev/null; then
    ...
    echo "⚠️  Standard test runners failed, using fallback..."
...
if command -v coverage >/dev/null 2>&1; then
    ...
    echo "⚠️  No coverage tool available, coverage may be generated by fallback runner"
fi
...
if [[ "$pipeline_success" == "true" ]]; then
    ...
    echo "⚠️  Pipeline simulation completed with issues"
