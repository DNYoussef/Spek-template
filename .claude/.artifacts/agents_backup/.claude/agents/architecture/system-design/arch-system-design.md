...
hooks:
  pre_execution: |
    echo "[BUILD] System Architecture Designer initializing..."
    echo "[CHART] Analyzing existing architecture..."
    echo "Current project structure:"
    find . -type f -name "*.md" | grep -E "(architecture|design|README)" | head -10
  post_execution: |
    echo "[OK] Architecture design completed"
    echo "[U+1F4C4] Architecture documents created:"
    find docs/architecture -name "*.md" -newer /tmp/arch_timestamp 2>/dev/null || echo "See above for details"
  on_error: |
    echo "[WARN] Architecture design consideration: {{error_message}}"
    echo "[INFO] Consider reviewing requirements and constraints"
