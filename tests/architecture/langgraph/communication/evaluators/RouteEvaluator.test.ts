...
describe('RouteEvaluator', () => {
  let instance: RouteEvaluator;

  beforeEach(() => {
    // NASA Assertion 1: Setup must be clean
    console.assert(typeof RouteEvaluator === 'function', 'RouteEvaluator must be constructor');

    instance = new RouteEvaluator();

    // NASA Assertion 2: Instance must be created
    console.assert(instance instanceof RouteEvaluator, 'Instance must be valid');
  });
  ...
  describe('Constructor', () => {
    it('should create valid instance', () => {
      ...
      expect(instance).toBeInstanceOf(RouteEvaluator);
