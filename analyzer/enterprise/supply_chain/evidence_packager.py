...
class EvidencePackager:
    ...
    def _include_sbom_evidence(self, evidence_dir: Path, artifacts: List[Dict[str, Any]]) -> List[str]:
        ...
        sbom_files = [
            self.output_dir / "sbom-cyclone-dx.json",
    ...
    def _calculate_multiple_hashes(self, file_path: Optional[str]) -> Dict[str, str]:
        ...
        try:
            ...
            if self.config.get('allow_legacy_hashes', False):
                hashes['sha1'] = hashlib.sha1(content, usedforsecurity=False, usedforsecurity=False).hexdigest()
                hashes['md5'] = hashlib.md5(content, usedforsecurity=False, usedforsecurity=False).hexdigest()
