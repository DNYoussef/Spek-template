#!/usr/bin/env python3
"""
DSPy Optimized Agent Template: planner
...
class PlannerSignature(dspy.Signature):
    """DSPy signature for planner with systematic optimization"""
...
class TransitionHub:
    ...
    def __init__(self):
        self.current_state = PlannerStates.INITIALIZING
        self.transition_history = []
        self.transition_count = 0  # Fixed bound for NASA compliance

    def transition(self, event: PlannerEvents) -> PlannerStates:
        """Execute state transition with fixed bounds (NASA Rule 10)"""
        # Assertion 1: Validate event type
        assert isinstance(event, PlannerEvents), "Event must be valid enum type"
        ...
        transition_matrix = {
            (PlannerStates.INITIALIZING, PlannerEvents.INITIALIZE): PlannerStates.READY,
            (PlannerStates.READY, PlannerEvents.START_PROCESSING): PlannerStates.PROCESSING,
            (PlannerStates.PROCESSING, PlannerEvents.COMPLETE_TASK): PlannerStates.COMPLETED,
            (PlannerStates.PROCESSING, PlannerEvents.HANDLE_ERROR): PlannerStates.ERROR,
            (PlannerStates.ERROR, PlannerEvents.INITIALIZE): PlannerStates.READY,
            (PlannerStates.COMPLETED, PlannerEvents.SHUTDOWN): PlannerStates.SHUTDOWN,
        }
...
class PlannerAgent(dspy.Module):
    """Production-ready planner agent with DSPy optimization"""
    ...
    def __init__(self):
        ...
        self.transition_hub.transition(PlannerEvents.INITIALIZE)
    ...
    def forward(self, task_description: str, context: Dict[str, Any], requirements: Dict[str, Any]) -> Dict[str, Any]:
        ...
        assert self.transition_hub.current_state == PlannerStates.READY, "Agent must be in READY state"

        try:
            # Transition to processing state
            self.transition_hub.transition(PlannerEvents.START_PROCESSING)
            ...
            self.transition_hub.transition(PlannerEvents.COMPLETE_TASK)
            ...
            self.transition_hub.transition(PlannerEvents.HANDLE_ERROR)
...
# SPECIALIZATION_CONSTRAINTS for planner
SPECIALIZATION_CONSTRAINTS = {
    'agent_type': 'project_manager',
    'category': 'planning',
    'model_assignment': 'GEMINI_FLASH',
    'capabilities': ["project_planning","resource_allocation","timeline_management"],
    'fsm_mode': 'optional',
...
if __name__ == "__main__":
    ...
    test_result = agent.forward(
        task_description="Execute planner specialized task with compliance",
        context={'agent_id': 'planner', 'category': 'planning'},
        requirements={'nasa_rule_10': True, 'fsm_patterns': True}
    )

    print(f"Agent planner test result: {test_result['compliance_status']}")
    print(f"Quality metrics: {test_result['quality_metrics']}")

# Template validation complete - 2025-09-28T17:56:23.576Z
