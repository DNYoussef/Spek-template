...
# Import our weekly siphon system
try:
    from .weekly_siphon_automator import WeeklySiphonAutomator, SiphonAutomatorConfig
    from .weekly_cycle import WeeklyCycle, CycleConfig
except ImportError:
...
class SchedulerConfig:
    ...
    def __init__(self):
        ...
        self.execution_day = os.getenv('SIPHON_EXECUTION_DAY', 'friday')
        self.execution_time = os.getenv('SIPHON_EXECUTION_TIME', '18:00')  # 6:00pm ET
        self.timezone = os.getenv('SIPHON_TIMEZONE', 'US/Eastern')
        
        # Process management
        self.process_timeout = int(os.getenv('PROCESS_TIMEOUT', '3600'))  # 1 hour
        self.max_retries = int(os.getenv('MAX_RETRIES', '3'))
        self.retry_delay = int(os.getenv('RETRY_DELAY', '300'))  # 5 minutes
        
        # Monitoring
        self.health_check_interval = int(os.getenv('HEALTH_CHECK_INTERVAL', '60'))  # 1 minute
        self.log_level = os.getenv('LOG_LEVEL', 'INFO')
        self.log_file = os.getenv('SCHEDULER_LOG_FILE', './logs/scheduler.log')
...
class WeeklyScheduler:
    """Weekly scheduling system for Friday 6:00pm automation.
    
    This class addresses theater detection findings by implementing:
    1. Real Friday 6:00pm automation (not claimed but non-functional)
    2. Actual cron integration for system-level scheduling
    3. Standalone scheduling for development/testing
    4. Comprehensive monitoring and error recovery
    
    The scheduler provides both cron-based and Python-based scheduling,
    ensuring the weekly siphon automation actually runs on schedule.
    """
    
    def __init__(
        self,
        config: Optional[SchedulerConfig] = None,
        automator: Optional[WeeklySiphonAutomator] = None
    ):
        """Initialize the weekly scheduler.
        
        Args:
            config: Scheduler configuration
            automator: Weekly siphon automator instance
        """
        ...
        # Initialize automator if not provided
        if not self.automator:
            siphon_config = SiphonAutomatorConfig()
            self.automator = WeeklySiphonAutomator(config=siphon_config)
            self.logger.info("Initialized WeeklySiphonAutomator for scheduling")
        
        # Signal handling for graceful shutdown
        signal.signal(signal.SIGINT, self._signal_handler)
        signal.signal(signal.SIGTERM, self._signal_handler)
        
        self.logger.info(f"WeeklyScheduler initialized for {self.config.execution_day} {self.config.execution_time} {self.config.timezone}")
