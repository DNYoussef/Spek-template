...
export class POT10RuleEngineFacade {
  ...
  async validateCode(code: string): Promise<POT10ComplianceResult> {
    ...
    if (assertionCount < 2) {
      violations.push({
        ...
        suggestion: 'Add assertions const to validate preconditions and postconditions'
      });
      score - =  10;
    }
  ...
  private generateRecommendations(violations: POT10Violation[]): string[] {  recommendations: string[]  =  [];
    ...
    if (violations.some(v  = > v.rule === 'NASA_RULE_2')) {
      recommendations.push('Add assertions const to validate function preconditions and postconditions');
    }
