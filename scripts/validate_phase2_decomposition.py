...
class Phase2ValidationRunner:
    ...
    def _validate_service_modules(self) -> Dict[str, Any]:
        ...
        for service_file in service_files:
            ...
            try:
                ...
                print(f"       [OK] {service_file}: {validation['loc']} LOC, {len(validation['classes'])} classes, {len(validation['methods'])} methods")
    ...
    def _validate_facade_compatibility(self) -> Dict[str, Any]:
        ...
        for facade_file in facade_files:
            ...
            try:
                ...
                print(f"       [OK] {facade_file}: Compatibility {compatibility_check['compatibility_score']:.0%}")
    ...
    def _validate_nasa_compliance(self) -> Dict[str, Any]:
        ...
        if nasa_results["compliance_score"] >= 0.92:
            ...
            print(f"       [OK] NASA Compliance: {nasa_results['compliance_score']:.1%} (>=92% required)")
        else:
            nasa_results["status"] = "FAILED"
            print(f"       [X] NASA Compliance: {nasa_results['compliance_score']:.1%} (>=92% required)")
    ...
    def _validate_loc_reduction(self) -> Dict[str, Any]:
        ...
        for module, metrics in expected_reductions.items():
            ...
            status = "[OK]" if reduction_result["meets_target"] else "[X]"
    ...
    def _generate_overall_assessment(self, validation_time: float) -> Dict[str, Any]:
        ...
        if failed_count > 0:
            overall_status = "FAILED"
            status_message = f"[FAIL] FAILED: {failed_count} critical issues found"
        elif warning_count > 0:
            overall_status = "WARNING"
            status_message = f"[WARNING]?  WARNING: {warning_count} issues need attention"
        else:
            overall_status = "PASSED"
            status_message = "[OK] PASSED: All Phase 2 decomposition validations successful"
