...
parse_gate_results() {
    if ! jq empty "$GATE_FILE" 2>/dev/null; then
        echo "❌ Invalid JSON in gate file: $GATE_FILE"
        exit 1
    fi
    ...
    if [[ -z "$failed_gates" ]]; then
        echo "❓ Gates marked as failed but no specific failures found"
        return 1
    fi
...
format_github_output() {
    ...
    cat <<EOF
## ❌ Quality Gates Failed ($gate_count gate$([ $gate_count -ne 1 ] && echo "s"))
...
format_plain_output() {
    ...
    echo "❌ Quality Gates Failed ($gate_count gate$([ $gate_count -ne 1 ] && echo "s"))"
    echo ""
...
create_pr_comment() {
    ...
    echo "📝 PR comment saved to: $comment_file"
    ...
    echo "📋 PR status saved to: $status_file"
}
