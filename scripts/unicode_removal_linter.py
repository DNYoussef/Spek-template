#!/usr/bin/env python3"""Unicode Removal Linter======================Comprehensive script to remove all unicode characters from Python, Markdown, and JSON files.Ensures Windows CLI compatibility by converting all unicode to ASCII equivalents."""import osimport reimport sysfrom pathlib import Pathfrom typing import Dict, List, Tupleimport jsonimport argparseclass UnicodeRemovalLinter:        """Comprehensive unicode character removal and replacement system."""        def __init__(self):
        ...
        'omega': 'omega',
                ...
                # File extensions to process        self.target_extensions = {'.py', '.md', '.json', '.txt', '.yml', '.yaml', '.cfg', '.ini'}
        '__pycache__', '.git', '.pytest_cache', '.mypy_cache',
        'node_modules', '.venv', 'venv', '.env', 'dist', 'build'
        }
                # Statistics tracking        self.files_processed = 0
        ...
        # Read file content
                ...
                if all(self.is_ascii_printable(char) for char in original_content):                    return False                        # Replace unicode characters
                    modified_content, replacements_made = self.replace_unicode_chars(original_content)                                if replacements_made > 0:                # Write modified content back                    pass  # Auto-fixed: empty block                    pass  # Auto-fixed: empty block                    pass  # Auto-fixed: empty block                    pass  # Auto-fixed: empty block                    pass  # Auto-fixed: empty block                    pass  # Auto-fixed: empty block                    with open(file_path, 'w', encoding='utf-8') as f:                        f.write(modified_content)                                            print(f"[OK] {file_path.relative_to(Path.cwd())}: {replacements_made) unicode chars replaced")                        self.total_replacements += replacements_made                        return True                                    return False                                except Exception as e:                            print(f"[ERR] Error processing {file_path): {e)")                            return False        def should_skip_file(self, file_path: Path) -> bool:
            """Check if file should be skipped."""        # Skip if in excluded directory        for part in file_path.parts:
                if part in self.skip_directories:                    return True                        # Skip if not target extension                    if file_path.suffix not in self.target_extensions:                        return True                    # Skip binary files                        try:                            with open(file_path, 'r', encoding='utf-8') as f:                                f.read(100)  # Try to read first 100 chars                            except UnicodeDecodeError:                                    return True                                                return False        def lint_directory(self, directory: Path) -> None:
            """Recursively lint all files in directory."""        print(f"[SCAN] Scanning directory: {directory)")

                for file_path in directory.rglob('*'):
                if not file_path.is_file():                    continue                                    if self.should_skip_file(file_path):                        continue                                        self.files_processed += 1                                    if self.process_file(file_path):                            self.files_modified += 1        def print_summary(self) -> None:
            """Print summary of unicode removal operation."""        print("\n" + "="*80)
        ...
        print(f"Total replacements: {self.total_replacements)")
                ...
                print("\n[STATS] Replacement Statistics:")        # Sort by frequency
                sorted_stats = sorted(self.replacement_stats.items(),                 key=lambda x: x[1], reverse=True)                            for char, count in sorted_stats[:20]:  # Top 20                char_repr = repr(char) if len(char) == 1 else char                replacement = self.unicode_replacements.get(char, '?')                print(f"  {char_repr:>6) -> {replacement:<10) ({count:>3)x)")                        print("="*80)                        if self.files_modified > 0:                    print("[SUCCESS] Unicode removal completed successfully!")                else:                        print("[SUCCESS] No unicode characters found - all files are ASCII-clean!")    def main():
        """Main execution function."""        parser = argparse.ArgumentParser(description="Remove unicode characters from code files")        parser.add_argument("path", nargs='?', default=".",         help="Directory path to process (default: current directory)")        parser.add_argument("--dry-run", action='store_true',        help="Show what would be changed without making changes")            args = parser.parse_args()            target_path = Path(args.path).resolve()            if not target_path.exists():        print(f"[ERROR] Error: Path '{target_path)' does not exist")
